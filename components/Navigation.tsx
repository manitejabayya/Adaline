...
const ProductsDropdown = ({ isOpen }: { isOpen: boolean }) => {
  ...
  return (
    <div 
      className={`fixed top-[60px] left-0 w-screen bg-[#e8e8e3] border-t border-b border-[#1a2e1a]/10 transition-all duration-300 ease-out ${
            ...
            return (
              <div 
                ...
                className={`flex flex-col items-start cursor-pointer transition-opacity duration-300 ${
                  hasHover ? (isActive ? 'opacity-100' : 'opacity-40') : 'opacity-100'
                }`}
              ...
              >
                <div className="relative w-full">
                  ...
                  <div className="absolute top-0 right-0 flex items-center gap-1">
                    <div 
                      className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-medium transition-colors duration-300 ${
                        isActive 
...
const Navigation = forwardRef<HTMLDivElement, NavigationProps>(
  function Navigation({ className = "" }, ref) {
